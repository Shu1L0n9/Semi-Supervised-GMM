function errstring = consist(model, type, inputs, outputs)
  ...
  if nargin > 2
    ...
    data_nin = double(size(inputs, 2));
    if double(model.nin) ~= data_nin
      errstring = ['Dimension of inputs ', num2str(data_nin), ...
      ' does not match number of model inputs ', num2str(double(model.nin))];
      return
    end
  end
  ...
  if nargin > 3
    ...
    data_nout = double(size(outputs, 2));
    if double(model.nout) ~= data_nout
      errstring = ['Dimension of outputs ', num2str(data_nout), ...
      ' does not match number of model outputs ', num2str(double(model.nout))];
      return
    end
  
  % Also check that number of data points in inputs and outputs is the same
    num_in = double(size(inputs, 1));
    num_out = double(size(outputs, 1));
    if num_in ~= num_out
